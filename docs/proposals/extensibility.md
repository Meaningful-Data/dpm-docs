...
!!! warning "Status: draft proposal for discussion"
    ...
    [Chapter 4](../ownership-documentation.md), in particular
    [Concept and Ownership (4.1.2)](../ownership-documentation.md#412-concept-and-ownership).
...
1. **Single ownership with inheritance**
   ([4.1.2](../ownership-documentation.md#412-concept-and-ownership)). Every Concept has exactly
...
2. **Owner-prefixed identifiers**
   ([4.1.2](../ownership-documentation.md#412-concept-and-ownership)). The first digits of every
...
3. **Code uniqueness is per Owner, not global**
   ([4.4](../ownership-documentation.md#44-naming-convention)): "one Category must not have two or
...
4. **Translations are already a cross-owner extension**
   ([4.1.3.1](../ownership-documentation.md#4131-translations)). Any Organisation can attach a
...
5. **ConceptRelation works across Owners by design**
   ([4.1.4](../ownership-documentation.md#414-concept-relation)): it "enables linking Concepts
...
6. **Releases and application dates** ([4.2](../ownership-documentation.md#42-historisation))
...
[4.2.3](../ownership-documentation.md#423-deactivations) and
[4.2.4](../ownership-documentation.md#424-dependencies)).
...
|---|---|---|---|
| DPMClass, DPMAttribute | Yes (implicit) | No | Fixed by the DPM metamodel authors (Owner: DPMM); Modellers must not edit ([4.1.1](../ownership-documentation.md#411-metamodel-metadata-entities)). Changes go through metamodel governance, not through model-level extension. |
...
| ConceptRelation.Type | Yes | **Yes** | Explicitly extensible per [4.1.4](../ownership-documentation.md#414-concept-relation): "Modellers can extend ConceptRelation.Type with other options". Free-text typed; collisions are tolerable because relations are annotations, not executable semantics. |
...
| ConceptRelation | n/a | **Yes — cross-owner by design** | Stated purpose includes "linking Concepts … across Owners" ([4.1.4](../ownership-documentation.md#414-concept-relation)). The relation itself is owned by whoever asserts it. |
...
| Entity | Reuse | Extend | Justification |
|---|---|---|---|
| **Category** | Yes (as glossary root, SubCategory parent, …) | **Yes — items, properties and hierarchy links** | The flagship extension case. A Category is an open vocabulary (P4): B adding national Items to A's "Instrument type" Category adds terms without changing any existing term. The naming convention already legislates for it (per-Owner code uniqueness *within* a Category, [4.4](../ownership-documentation.md#44-naming-convention)). Mechanism: B-owned Items + B-owned ItemCategory links; B may also place its Items under A's Items in the hierarchy (the ParentItem link lives in B's ItemCategory row). Scoped resolution (§4.1) guarantees A's open Cells, domains and aggregations are unaffected in A's own modules. |
...
4. **Dependency pinning made explicit.** Cross-owner references and extensions are only valid if
   ...
   [4.2.4](../ownership-documentation.md#424-dependencies) (no new use of ended/deactivated
